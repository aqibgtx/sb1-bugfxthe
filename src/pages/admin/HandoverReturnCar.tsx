...
import { getMalaysiaTime, getMalaysiaDateTimeString } from '../../lib/timezone';
...
const AdminHandoverReturnModal: React.FC<{
  selectedBooking: any;
  modalType: 'handover' | 'return';
  onClose: () => void;
  onConfirm: (photoUrl: string, actionTime: string, lateFee?: number, invoiceData?: any) => void;
  processing: boolean;
}> = ({ selectedBooking, modalType, onClose, onConfirm, processing }) => {
  ...
  // Calculate overdue info with custom time
  const calculateOverdueInfo = () => {
    if (modalType !== 'return' || !selectedBooking.handover_time) {
      return { isOverdue: false, hoursOverdue: 0, lateFee: 0 };
    }

    const returnTime = new Date(actionTime);
    const handover = new Date(selectedBooking.handover_time);
    const expectedReturn = new Date(handover);
    expectedReturn.setDate(expectedReturn.getDate() + selectedBooking.total_days);
    ...
    const lateFee = isOverdue ? Math.ceil(hoursOverdue) * dailyRate * 0.1 : 0;
    
    return { isOverdue, hoursOverdue: Math.max(0, hoursOverdue), lateFee, expectedReturn };
  };

  const overdueInfo = calculateOverdueInfo();

  // Rest of the modal implementation similar to HandoverReturnModal but with editable time
  // ... (implementation details similar to original modal)
  
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      {/* Modal content with editable time field for admin */}
      <motion.div
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-2xl font-bold text-gray-900">
              Admin: {modalType === 'handover' ? 'Vehicle Handover' : 'Vehicle Return'} Verification
            </h3>
          ...
          <div className="space-y-6">
            ...
            {/* Editable Action Time for Admin */}
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">
                {modalType === 'handover' ? 'Handover' : 'Return'} Time (Admin Editable)
              </label>
              <input
                type="datetime-local"
                value={actionTime}
              ...
              <p className="text-xs text-gray-500">
                As admin, you can set custom {modalType} time. Default is current Kuala Lumpur time.
              </p>
            </div>

            {/* Rest of modal content similar to original */}
            {/* ... */}
          </div>
        </div>
      </motion.div>
...
const AdminHandoverReturnCar: React.FC = () => {
  const { user } = useAuth();
  ...
  // Admin can see ALL bookings, not just their own
  const loadBookings = useCallback(async () => {
    ...
    try {
      ...
      // Admin sees ALL bookings (no staff_id filter)
      const handover = bookings.filter(booking => 
        booking.booking_status === 'approved' && 
        !booking.handover_marked
      );
      
      // Admin sees ALL active rentals (no staff_id filter)
      // Any admin can process returns from any staff/customer
  ...
  // Pagination helpers (same as staff version)
  ...
  // Admin version of confirm action with custom time support
  const handleConfirmAction = async (photoUrl: string, actionTime: string, lateFee?: number, lateFeeInvoiceData?: any, depositData?: any, returnData?: any, currentMileage?: number, handoverReturnInvoiceData?: any, lateFeePaymentMethod?: string) => {
    ...
    if (!selectedBooking?.id || typeof selectedBooking.id !== 'string' || selectedBooking.id.length !== 36) {
    ...
    try {
      if (modalType === 'handover') {
        // Ensure user.id is a complete UUID string
        const handoverById = user.id.toString().trim();
        if (handoverById.length !== 36) {
          throw new Error('Invalid user ID format for handover');
        }
        ...
        // Show invoice share modal instead of simple confirmation
        if (handoverReturnInvoiceData) {
        ...
        // Ensure user.id is a complete UUID string
        const returnedById = user.id.toString().trim();
        if (returnedById.length !== 36) {
          throw new Error('Invalid user ID format for return');
        }
          ...
          try {
            const { error: paymentError } = await supabase
              ...
              .insert({
                ...
                notes: `Late fee payment generated by admin during vehicle return process. Amount: RM ${lateFee}`,
            ...
            if (paymentError) {
              ...
              console.log('Late fee payment record created successfully by admin');
            }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Admin: Handover & Return Cars</h1>
          <p className="text-gray-700">Manage all car handovers and returns across all staff and customers with admin privileges.</p>
        </div>
        <Button
      ...
      {/* Admin Notice */}
      <Card className="border-l-4 border-purple-500 bg-purple-50 border border-purple-200">
        <div className="flex items-center space-x-3 mb-3">
          <CheckCircle className="w-6 h-6 text-purple-500" />
          <h3 className="text-purple-800 font-semibold">Admin Access</h3>
        </div>
        <div className="text-purple-700 text-sm">
          <p>As an admin, you have <strong>full rights</strong> to view and manage handovers/returns for <strong>all bookings</strong> across all staff members. You also have the ability to set custom times for handover and return actions.</p>
        </div>
      </Card>
            ...
            {paginatedHandoverBookings.length > 0 ? (
              <div className="space-y-4">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h2 className="text-xl font-semibold text-gray-900">Ready for Handover (All Staff)</h2>
            ...
            {paginatedReturnBookings.length > 0 ? (
              <div className="space-y-4">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h2 className="text-xl font-semibold text-gray-900">Active Rentals (All Staff)</h2>
...
export default AdminHandoverReturnCar;