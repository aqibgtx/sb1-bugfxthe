...
const AdminBookings: React.FC = () => {
  ...
  const { bookings, filterCounts, overdueBookings } = useMemo(() => {
    ...
    const counts = {
      ...
      pending_approval: allBookings.filter(b => b.booking_status === 'pending_approval').length,
      approved: allBookings.filter(b => b.booking_status === 'approved').length,
      ongoing: allBookings.filter(b => b.booking_status === 'ongoing').length,
      extended: allBookings.filter(b => b.booking_status === 'extended').length,
      handed_over: allBookings.filter(b => b.booking_status === 'handed_over').length,
    ...
    if (activeFilter === 'overdue') {
      ...
      filteredBookings = allBookings.filter(booking => booking.booking_status === activeFilter);
    }
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      if (bookingsResult) {
        ...
        setPagination(prev => ({
          ...prev,
          totalItems: bookingsResult.data?.length || 0
        }));
      } else {
  ...
  const handleAdminDepositDeduction = async (deductionData: {
    ...
    try {
      ...
      const currentDeducted = selectedBooking.deposit_deducted || 0;
      const newTotalDeducted = currentDeducted + deductionData.requestedAmount;
      
      const { error } = await supabase
        .from('bookings')
        .update({
          deposit_deducted: newTotalDeducted,
          deposit_deduction_reason: deductionData.reason,
      ...
      toast.success(`Deposit deduction of RM ${deductionData.requestedAmount} applied successfully`);
  ...
  const StatusFilter = () => {
    const filters = [
      ...
      { key: 'ongoing', label: 'Ongoing', count: filterCounts.ongoing || 0 },
    ...
    >
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-700 text-sm md:text-base">Manage all rental bookings and track their status</p>
        </div>
        <Button
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <p><span className="text-gray-600">Staff:</span> <span className="text-gray-900">{booking.staff?.name}</span></p>
              ...
              {paginatedBookings.map((booking, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 px-4">
                      <span className="text-gray-900">{booking.staff?.name}</span>
                    </td>
                    <td className="py-4 px-4">
      ...
      {showDepositDeductionModal && selectedBooking && (
        <DepositDeductionModal
          ...
          booking={selectedBooking}
          onSubmit={handleAdminDepositDeduction}
...
// Admin Booking Details Modal Component
const AdminBookingDetailsModal: React.FC<{
  booking: any;
  onClose: () => void;
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-4 p-4 sm:p-6 border-t border-gray-200 flex-shrink-0 bg-gray-50">
            {['approved', 'ongoing', 'extended', 'handed_over'].includes(booking.booking_status) && (
              ...
              >
                <Plus className="w-4 h-4" />
                <span>{booking.booking_status === 'extended' ? 'Extend Again' : 'Extend Booking'}</span>
              </Button>
            )}
