...
export const useClientBookingForm = () => {
  const location = useLocation();
  const { user } = useAuth();
  
  // State
  const [currentStep, setCurrentStep] = useState(1);
  const [selectedCar, setSelectedCar] = useState<any>(location.state?.selectedCar || null);
  const [selectedCustomer, setSelectedCustomer] = useState<any>(user); // Client is always the customer
  const [bookingFor, setBookingFor] = useState('myself'); // Client always booking for themselves
  ...
  // Fetch data functions (simplified and direct)
  ...
  // Load initial data (simplified approach)
  const loadInitialData = async () => {
    try {
      ...
      const [carsResult, addOnsResult] = await Promise.allSettled([
        fetchCars(),
        fetchAddOns()
      ]);
      ...
      // Set customers to just the current user
      setCustomers([user]);
  ...
  const handleCustomerSelect = (customer: any) => {
    // For client, this is always the current user
    console.log('Customer selected (always current user):', user);
    setSelectedCustomer(user);
  };
  ...
  const handleSubmitBookingRequest = async () => {
    ...
    try {
      ...
      const bookingData = {
        customer_id: user.id,
        staff_id: null, // Client bookings don't have staff assistance
        ...
        notes: `${deliveryType !== 'self_pickup' ? `Delivery: ${deliveryType} (${Math.round(deliveryDistance)}km) | ` : ''}Booking for: ${bookingFor} | Payment method: ${getPaymentMethodCode(paymentMethod)} | Self-booking by customer (no staff assistance), awaiting admin approval and payment processing`
      };
  ...
  const canProceedToStep = (step: number) => {
    switch (step) {
      ...
      case 3:
        return bookingDetails.startDate && bookingDetails.totalDays > 0;
      case 4:
        return true;
      case 5:
