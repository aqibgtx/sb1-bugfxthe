...
import { AgentBookingData, AgentBookingState, AgentBookingActions } from '../types/agentBooking';
import { getMalaysiaDateString, addDaysInMalaysiaTime } from '../lib/timezone';
import toast from 'react-hot-toast';

export const useAgentBooking = (): AgentBookingData & AgentBookingState & AgentBookingActions => {
  // State
  ...
  useEffect(() => {
    if (bookingDetails.startDate && bookingDetails.totalDays > 0) {
      ...
      // Use custom price if requested, otherwise use car's daily rate
  ...
  // Pre-select staff user as customer
  useEffect(() => {
    if (user && customers.length > 0) {
      ...
      if (staffAsCustomer) {
        ...
        // If staff user doesn't exist as customer, create a temporary customer object
        const tempCustomer = {
          id: user.id,
          name: user.name,
          email: user.email,
  ...
  const handleSubmitAgentBookingRequest = async () => {
    ...
    try {
      // Step 1: Create VIP/Agent booking request
      const bookingData = {
      ...
      // Step 3: Add delivery details if enabled
      ...
      if (paymentMethod === 'cash' && receiptFile) {
        try {
          ...
          // Create payment record with VIP/Agent marking
          const { error: paymentError } = await supabase
            ...
            .insert({
              ...
              approved: false,
              is_agent_booking: true, // Mark payment as VIP/Agent
              custom_price_requested: customPriceRequested > 0 ? customPriceRequested : null,
              agent_notes: agentNotes.trim() || null
            });
  ...
  const resetForm = () => {
    setCurrentStep(1);
    setSelectedCar(null);
    // Don't reset selectedCustomer as it should remain pre-selected
  ...
  return {
    ...
    handleSubmitAgentBookingRequest,
    resetForm,
