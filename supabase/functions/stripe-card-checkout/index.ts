...
const stripe = new Stripe(stripeSecret, {
  appInfo: {
    name: 'Budget Plus Rental - Card Integration',
    version: '1.0.0',
  },
});
...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`Created card checkout session ${session.id} for booking ${booking_id}`);
    ...
    console.error(`Card checkout error: ${error.message}`);
