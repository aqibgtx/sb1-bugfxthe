...
const stripe = new Stripe(stripeSecret, {
  appInfo: {
    name: 'Budget Plus Rental - FPX Integration',
    version: '1.0.0',
  },
});
...
Deno.serve(async (req) => {
  try {
    ...
    // Create Stripe checkout session for FPX
    ...
    console.log(`Created FPX checkout session ${session.id} for booking ${booking_id}`);
    ...
    console.error(`FPX checkout error: ${error.message}`);
